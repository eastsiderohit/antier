...
resource "null_resource" "setup_kube_config" {
  provisioner "local-exec" {
    command = "aws eks --region ${local.environments[terraform.workspace].region} update-kubeconfig --name ${module.eks.cluster_name} --profile ${local.environments[terraform.workspace].account_id} "
  }
}
